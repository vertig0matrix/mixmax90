...
describe("DB Connection", () => {
    ...
    test("GET /toptracks status", async () => {
    ...
    test("GET /toptracks response body", async () => {
        const res = await request(app).get("/toptracks")
        expect(res.body[0]._id).toEqual("65aff9ded6fd888b4c686e6a")
    });

    test("POST /toptracks status", async () => {
            const res = await request(app).post("/toptracks")
            .send(object)
    ...
    test("POST /toptracks post to db", async () => {
            await request(app).post("/toptracks")
            .send(res_obj)
            const getRes = await request(app).get("/toptracks")
